...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option.setName("code").setDescription("Enter Code:").setRequired(true)
        ),
    ...
    async execute(interaction, client) {
        ...
        if (adminRole) {
            const data = { code: Icode }

            await axios.post(process.env.Dash_URL + '/api/revokecoupon', data, {
                headers: {
                ...
                if (response.data.status == 'success') {
                    const embed = new EmbedBuilder()
                        ...
                        .setDescription(
                            "Successfuly Deleted the Coupon:  [`" + Icode + "`]   :white_check_mark:"
                        )
                        .setThumbnail(
                            process.env.Icon
                        )
